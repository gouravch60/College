...
export const getAllDepartment=(limit)=>dispatch=>{
    ...
    let link ="/api/v1/department/"
    if(limit){
        link =`/api/v1/department/?limit=${limit}`
    }
    else{
        link ="/api/v1/department/"
    }
...
export const newDepartment=(data)=>dispatch=>{
    ...
    let link ="/api/v1/department/new";
    const config = {
...
export const updateDeptPic=(id,data)=>dispatch=>{
    ...
    let link ="/api/v1/department/imageupdate/"+id;
    console.log(link);
    const config = {
        headers: { "Content-Type": 'multipart/form-data' },
      };
